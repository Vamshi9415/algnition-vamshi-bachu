...
`run.sh` is the single grading entry point: it generates features from `DATA_DIR` and produces predictions from `MODEL_PATH`, writing to `OUTPUT_PATH`. A trained model bundle is already committed at `pickle/model.pkl`, so by default it **loads and reuses that model rather than retraining**.
...
# Runs end-to-end with defaults: DATA_DIR=data MODEL_PATH=pickle/model.pkl OUTPUT_PATH=output/predictions.csv
./run.sh
# equivalently, with explicit args:
...
`data/` holds the sample ad-platform CSVs (`google_ads_campaign_stats.csv`, `meta_ads_campaign_stats.csv`, `bing_campaign_stats.csv`) — replace its contents with new data of the same schema and re-run to forecast against it; the code reads whatever CSVs are present, it doesn't hardcode filenames.

### Retraining

To retrain instead of reusing the committed bundle, pass a `MODEL_PATH` that doesn't exist yet (or delete `pickle/model.pkl` first):

```bash
SKIP_TRAIN=0 ./run.sh ./data ./pickle/model_new.pkl ./output/predictions.csv
```

### Live dashboard (separate from grading)

The FastAPI/React dashboard (see [Development Setup](#development-setup)) is a separate, optional demo layer. It loads its own model copy at `ml_engine/pickle/model.pkl` (kept in sync with `pickle/model.pkl`) and serves the checked-in historical CSVs at the repo root — no upload step needed. If you retrain and want the live app to reflect it, copy the new bundle to `ml_engine/pickle/model.pkl` too.
...
| `DATA_DIR` | `data` | Directory with raw ad-platform CSVs |
| `MODEL_PATH` | `pickle/model.pkl` | Save/load path for the trained model bundle |
...
│   └── tests/
├── data/                       Grading input folder — sample CSVs; overwritten with held-out data at test time
├── pickle/
│   └── model.pkl               Committed, already-trained model bundle (grading loads this, does not retrain)
├── output/                     Generated by run.sh — predictions.csv written fresh each run
