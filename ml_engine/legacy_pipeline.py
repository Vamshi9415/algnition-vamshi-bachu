...
from ml_engine.features.store import FeatureStore
...
from ml_engine.paths import output_path
...
class AIgnitionPipeline:
    ...
    Usage:
        ...
        result = pipeline.run(["ml_engine/data/raw/google.csv", "ml_engine/data/raw/bing.csv"])
    """

    def __init__(self, horizon: int = 60, output_dir: str | Path | None = None):
    ...
    def run(self, filepaths: list[str]) -> dict:
        ...
        ai_summary = self.llm.executive_summary(summary)
        budget_rec = self.llm.budget_recommendation(sim_results)
