...
from .gemini import GeminiError, GeminiLLM, GENAI_AVAILABLE


class InsightGenerator:
    """Generates AI-powered executive summaries and risk analyses using Gemini."""
    ...
    def __init__(self, model: str = "gemini-2.5-flash"):
        ...
        if GENAI_AVAILABLE:
            try:
                self.client = GeminiLLM(model=model)
            except ValueError:
                logger.warning("GEMINI_API_KEY not set — AI insights disabled")
            except Exception as e:
                logger.warning(f"Gemini client init failed: {e}")
        else:
            logger.warning("google-genai is not installed — AI insights disabled")

    def _call(self, system_prompt: str, user_content: str) -> str:
        if not self.client:
            return "[LLM unavailable — set GEMINI_API_KEY to enable AI insights]"
        try:
            return self.client.generate(user_content, system_prompt=system_prompt)
