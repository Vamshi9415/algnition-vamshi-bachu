...
import pandas as pd
import pytest
from ml_engine.canonical.schema import CanonicalSchemaBuilder, CANONICAL_COLUMNS
from ml_engine.ingestion.detector import Channel
...
def test_roas_computed(bing_df):
    # ROAS is derived downstream by DataCleaner, not by the canonical schema builder itself.
    builder = CanonicalSchemaBuilder()
    canonical = builder.build(bing_df, Channel.MICROSOFT)
    result = DataCleaner().clean(canonical)
