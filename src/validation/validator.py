...
class ValidationReport:
    ...
    def to_json(self, path: str = "reports/validation_report.json"):
        Path(path).parent.mkdir(parents=True, exist_ok=True)
        Path(path).write_text(json.dumps(self.to_dict(), indent=2, default=str))
        logger.info(f"Validation JSON written: {path}")

    def to_html(self, path: str = "reports/validation_report.html"):
...
<h3>Warnings ({len(self.warnings)})</h3><ul>{warnings_html}</ul>
        ...
        logger.info(f"Validation HTML written: {path}")
